...
The primary WiiM Mini is `wiim.example` and runs
`<FIRMWARE_VERSION>`. secondary WiiM at `secondary-wiim.example` is a separate WiiM and is not
monitored by this automation.

The primary unit exposes its local HTTP API over HTTPS with a self-signed
certificate. Plain HTTP requests fail. Its device information reports
`securemode=1` and `security=https/2.0`, so the local configuration uses:
